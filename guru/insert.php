...
if(isset($_POST['simpan'])){
    if ($pdo->query("select count(id_nilai) from nilai_mapel where id_murid='".$_GET["nisn"]."'")->fetchColumn() == 0) {
        $nisn=$_GET["nisn"];
        $id_mapel=$_POST["id_mapel"];
        $nilai=$_POST["nilai"];
        $i=0;
        foreach($id_mapel as $row){
            $insql = "insert into nilai_mapel(id_mapel, id_murid, nilai) values(:id_mapel, :NISN, :nilai)";
...
if(isset($_POST['update'])){
    ...
    $i=0;
    foreach($id_mapel as $row){
        $upsql = "update nilai_mapel set nilai = :nilai where id_mapel = :id_mapel and id_murid = :NISN";
    ...
    <body>
        ...
        <div class="wrapper">
          <!-- Sidebar Constructor -->
            <div id="murid" class="constructor">
                <div class="well well-sm" style="margin-top:5px">
                        <ul class="breadcrumb" style="margin-bottom:5px; background-color:#fff;">
                            ...
                            <li><a href="rapor.php">Nilai Mata Pelajaran</a></li>
            ...
            ?>
                <div class="panel">
                    <form class="form-horizontal" action="insert.php?nisn=<?php echo $_GET["nisn"]; ?>" method="post">
                        ...
                        <div class="panel-body">
                                ...
                                <fieldset>
                                    <?php
                                        $i=0;
                                        foreach($pdo->query("select * from mata_pelajaran") as $row){
                                    ?>
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10">
                                            ...
                                            <input type="number" class="form-control" id="inputEmail" placeholder="nilai" name="nilai[<?php echo $i; ?>]" value="<?php echo $pdo->query("select nilai from nilai_mapel where id_murid='".$_GET["nisn"]."' and id_mapel='".$row[0]."'")->fetchColumn(); ?>">
                                        </div>
                                    </div>
                                    <?php
                                            $i++;
